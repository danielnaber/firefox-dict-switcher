...
function changeDictionary(langCode, langName)
{
    ...
    if(dictionary)
    {
        message = "Detected " + dictionary;
        feedbackData.language = dictionary;
    }
    else
    {
        message = "Detected " + langCode + " but didn't find suitable dictionary";
        feedbackData.language = langCode;
        feedbackData.isWarning = true;
    }
    feedbackData.message = message;
    // TODO:
    /*if (langName && langCode.length == 2) {
        dictSwitcherButton.label = message;
        let icon16 = "./img/dictionary16-" + langCode + ".png";
        //let icon32 = "./img/dictionary32-" + langCode + ".png";
        dictSwitcherButton.icon = {
            "16": icon16
            //"32": icon32
        };
    }*/
...
// TODO: we need icons - one per language or just checkmark and question mark?
// Once this works, remove the feedback that manipulates the page DOM
/*var dictSwitcherButton = ActionButton({
    id: "lt-check",
    ...
    onClick: function(state) {
        // there's no real action, so show some help text 
    }
});*/
