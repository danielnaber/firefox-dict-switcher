...
function showPanel(e)
{
    ...
    //    dialog = mainWindow.openDialog(data.url("ui/options.html"),
    //                      "Dictionary settings",
    //                      "chrome,centerscreen,dialog,modal,resizable,scrollbars,height=400,width=400",
    //                      JSON.stringify(parsedDictionaries));
    tabs.open({
...
function parseAvailableDictionaries()
{
    ...
    for(let i = 0; i < availableDictionaries.length; i++)
    {
        ...
        let dialect = availableDictionaries[i],
            ...
            languageName,
            // This is the freindly full dictionary name including country name if available. Examples are:
...
function changeDictionary(language)
{
    ...
    /*if(parsedDictionaries.preferenceList.indexOf(language) != -1)
        dictionary = parsedDictionaries["pref_" + language];
    else if(parsedDictionaries[language] !== undefined && parsedDictionaries[language].length > 1)
    {
        ...
        dictionary = parsedDictionaries[language][1].code;
    }*/
    ...
    // language to the language of the Firefox locale. Hopefully this causes the least annoyance
    if(!languageData)
        spellchecker.dictionary = appLocale;
    else
    {
        ...
        // If we find a preferred dictionary (a per what the user configured) use it, otherwise, use the first
